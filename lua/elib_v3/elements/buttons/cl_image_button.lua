...
function PANEL:Init()
    ...
    self:SetImageURL(Elib.ProgressImageURL)
...
function PANEL:Paint(w, h)
    ...
    if not self:IsEnabled() then
        Elib.DrawImage(imageOffset, imageOffset, imageSize, imageSize, self:GetImageURL(), self:GetDisabledColor())
        return
    end
    ...
    self.ImageCol = Elib.LerpColor(FrameTime() * 12, self.ImageCol, col)

    Elib.DrawImage(imageOffset, imageOffset, imageSize, imageSize, self:GetImageURL(), self.ImageCol)
end

vgui.Register("Elib.ImageButton", PANEL, "Elib.Button")