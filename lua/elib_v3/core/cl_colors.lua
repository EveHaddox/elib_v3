...
do
    ...
    local max = math.max
    local min = math.min
    function Elib.ColorToHex(color)
...
function Elib.IsColorEqualTo(from, to)
...
if not colorMeta.Lerp then
    ...
    function colorMeta:Lerp(target, fraction)
        if isColor(fraction) then
            -- Don't break addons using this based on Elib UI for now.
            ...
            if not deprecation_warning_shown then
                ...
                ErrorNoHaltWithStack("Deprecated Elib-UI Color:Lerp(fraction, target) is used.")
            end
        end
