...
local setFont = Elib.SetFont
local getTextSize = Elib.GetTextSize
local setTextPos = surface.SetTextPos
local setTextColor = surface.SetTextColor
local drawText = surface.DrawText

function Elib.DrawSimpleText(text, font, x, y, col, xAlign, yAlign)
    setFont(font)
...
local drawSimpleText = Elib.DrawSimpleText
local gmatch = string.gmatch
local find = string.find
local max = math.max
local select = select

function Elib.DrawText(text, font, x, y, col, xAlign, yAlign)
    local curX = x
    local curY = y
...
function Elib.DrawShadowText(text, font, x, y, col, xAlign, yAlign, depth, shadow)
    shadow = shadow or 50
...
local drawShadowText = Elib.DrawShadowText

function Elib.DrawDualText(title, subtitle, x, y, h)
    x = x or 0
    y = y or 0
...
function Elib.WrapText(text, width, font) --Edit of https://github.com/FPtje/DarkRP/blob/master/gamemode/modules/base/cl_util.lua#L21
...
function Elib.EllipsesText(text, width, font)
    local chachedName = text .. width .. font
