...
do --Input handling
    ...
    hook.Add("PreRender", "Elib.UI3D2D.InputHandler", function() --Check the input state before rendering UIs
...
do --Rendering context creation and mouse position getters
    ...
    hook.Add("PreRender", "Elib.UI3D2D.GetLocalPlayer", function() --Keep getting the local player until it's available
        localPlayer = LocalPlayer()
        if IsValid(localPlayer) then hook.Remove("PreRender", "Elib.UI3D2D.GetLocalPlayer") end
    end)
    ...
    Elib.GetImage("https://pixel-cdn.lythium.dev/i/cyf6d6gzf", function(mat) cursorMat = mat end)
    Elib.GetImage("https://pixel-cdn.lythium.dev/i/m3m6x59yb", function(mat) cursorHoverMat = mat end)
...
hook.Run("Elib.UI3D2D.FullyLoaded")
